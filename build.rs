...
fn main() {
    // run bindgen to generate the rust call interfaces
    println!("cargo:rerun-if-changed=gmsh/api");
    let bindings = bindgen::Builder::default()
        .header("gmsh/api/wrapper.h")
        .generate_comments(true)
        .generate()
        .expect("Unable to generate bindings");

    // store the generated file "target/[debug|release]/gmsh-sys-[id]-out/bindings.rs"
